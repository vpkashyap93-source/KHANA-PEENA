...
function FestivalBanner({ festival, restaurantName }) { if (!festival) return null; return <div className={`festival-banner festival-${festival.theme}`}><span className="festival-emoji">{festival.emoji}</span><div><strong>{festival.message}</strong><small>From all of us at {restaurantName || 'Basil & Bean'}</small></div></div> }
...
function App() {
  ...
  return (
    <>
      ...
      <aside className={`sidebar ${mobileNav ? 'open' : ''}`}><div className="brand"><span className="brand-mark">{profile.restaurantName?.trim()?.[0]?.toUpperCase() || 'B'}</span><span><strong>{profile.restaurantName || 'BASIL & BEAN'}</strong><small>RESTAURANT OS</small></span><button className="icon-button sidebar-close" onClick={() => setMobileNav(false)}>×</button></div><div className="workspace-label">WORKSPACE</div><nav>
  {navItems
    .filter(([label]) => {
