...
function FestivalBanner({ festival }) { if (!festival) return null; return <div className={`festival-banner festival-${festival.theme}`}><span className="festival-emoji">{festival.emoji}</span><div><strong>{festival.message}</strong><small>From all of us at Basil &amp; Bean</small></div></div> }
...
const exportBackup = () => {
  ...
  link.href = url
  link.download = `basil-bean-backup-${new Date().toISOString().slice(0, 10)}.json`
  link.click()
    ...
    <div className="bill-modal bill-size-80">
      ...
      <div className="print-bill">
        <header className="bill-header">
          <h1>{profile?.restaurantName || 'BASIL & BEAN'}</h1>
...
function App() {
  ...
  const savePurchase = (purchase) => { const existing = purchases.find((item) => item.id === purchase.id); setPurchases((current) => existing ? current.map((item) => item.id === purchase.id ? purchase : item) : [purchase, ...current]); setInventory((current) => current.map((item) => item.id === purchase.inventoryId ? { ...item, currentStock: item.currentStock + Number(purchase.quantity) - (existing ? Number(existing.quantity) : 0) } : item)); notify(existing ? 'Purchase updated' : 'Purchase saved') }
  ...
  return (
    <>
      ...
      <aside className={`sidebar ${mobileNav ? 'open' : ''}`}><div className="brand"><span className="brand-mark">B</span><span><strong>BASIL & BEAN</strong><small>RESTAURANT OS</small></span><button className="icon-button sidebar-close" onClick={() => setMobileNav(false)}>×</button></div><div className="workspace-label">WORKSPACE</div><nav>
  {navItems
    .filter(([label]) => {
    ...
    .map(([label, glyph]) => (
...
</nav> <div className="sidebar-bottom"><div className="help-card"><span>?</span><div><strong>Need a hand?</strong><small>Visit our help center</small></div></div><div className="profile"><div className="avatar">{ownerInitials}</div><div><strong>{ownerName}</strong><small>Administrator</small></div><span>•••</span></div></div></aside>
      ...
      </main>{active === 'Inventory' && <InventoryPage inventory={inventory} setInventory={setInventory} notify={notify} />}{active === 'Purchases' && <PurchasePage inventory={inventory} suppliers={suppliers} purchases={purchases} savePurchase={savePurchase} notify={notify} />}{active === 'Suppliers' && <SupplierPage suppliers={suppliers} setSuppliers={setSuppliers} notify={notify} />}{active === 'Recipes' && <RecipePage menu={menu} inventory={inventory} recipes={recipes} setRecipes={setRecipes} notify={notify} />}{active === 'Stock Adjustments' && <AdjustmentPage inventory={inventory} adjustments={adjustments} adjustStock={adjustStock} notify={notify} />}{toast && <div className="toast">✓ {toast}</div>}
      </div>
    </>
  )
}
...
function Dashboard({ navigate, orders, tables, customers, now, festival, ownerName }) {
...
function ReportsPage({ orders, inventory }) { const totalSales = orders.reduce((sum, order) => sum + Number(order.amount || 0), 0); const totalGst = orders.reduce((sum, order) => sum + Number(order.gst || 0), 0); const paymentBreakup = ['Cash', 'UPI', 'Card'].map((method) => ({ method, total: orders.filter((order) => order.paymentMethod === method).reduce((sum, order) => sum + Number(order.paidAmount || 0), 0) })); const itemSales = {}; orders.forEach((order) => (order.items || []).forEach((line) => { const key = line.name; itemSales[key] = itemSales[key] || { name: line.name, category: line.category, sold: 0 }; itemSales[key].sold += line.quantity })); const topItems = Object.values(itemSales).sort((a, b) => b.sold - a.sold); const lowStock = inventory.filter((item) => item.currentStock <= item.minimumStock); return <><PageTitle eyebrow="REPORTS" title="Business reports" /><div className="summary-grid"><article className="summary-card"><span>Total sales</span><strong>{money(totalSales)}</strong></article><article className="summary-card"><span>GST collected</span><strong>{money(totalGst)}</strong></article><article className="summary-card"><span>Total orders</span><strong>{orders.length}</strong></article><article className="summary-card"><span>Low stock items</span><strong>{lowStock.length}</strong></article></div><section className="panel"><div className="panel-heading"><div><span className="eyebrow">SALES BY PAYMENT METHOD</span><h2>Collections</h2></div></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Method</th><th>Amount</th></tr></thead><tbody>{paymentBreakup.map((row) => <tr key={row.method}><td>{row.method}</td><td>{money(row.total)}</td></tr>)}</tbody></table></div></section><section className="panel"><div className="panel-heading"><div><span className="eyebrow">ITEM-WISE SALES</span><h2>Top selling items</h2></div></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Item</th><th>Qty sold</th></tr></thead><tbody>{topItems.length ? topItems.map((row) => <tr key={row.name}><td>{row.name}</td><td>{row.sold}</td></tr>) : <tr><td colSpan="2">No sales recorded yet.</td></tr>}</tbody></table></div></section>{lowStock.length > 0 && <section className="panel"><div className="panel-heading"><div><span className="eyebrow">STOCK ALERT</span><h2>Low stock items</h2></div></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Item</th><th>Current stock</th><th>Minimum</th></tr></thead><tbody>{lowStock.map((item) => <tr key={item.id}><td>{item.name}</td><td>{item.currentStock} {item.unit}</td><td>{item.minimumStock} {item.unit}</td></tr>)}</tbody></table></div></section>}</> }
...
function SimpleForm({ title, fields, form, setForm, onSave, onCancel }) { return <div className="panel module-editor"><h2>{title}</h2><div className="settings-form-grid">{fields.map((field) => <label key={field}>{field.replace(/([A-Z])/g, ' $1')}<input value={form[field] ?? ''} onChange={(event) => setForm({ ...form, [field]: event.target.value })} /></label>)}</div><div className="cart-actions"><button className="button secondary" onClick={onCancel}>Cancel</button><button className="button primary" onClick={onSave}>Save</button></div></div> }
function InventoryPage({ inventory, setInventory, notify }) { const [search, setSearch] = useState(''); const [form, setForm] = useState(null); const save = () => { const item = { ...form, id: form.id || `raw-${Date.now()}`, openingStock: Number(form.openingStock) || 0, currentStock: Number(form.currentStock ?? form.openingStock) || 0, minimumStock: Number(form.minimumStock) || 0, costPrice: Number(form.costPrice) || 0 }; setInventory((current) => current.some((entry) => entry.id === item.id) ? current.map((entry) => entry.id === item.id ? item : entry) : [...current, item]); setForm(null); notify('Raw material saved') }; return <ModuleFrame title="Inventory" action="Add raw material" onAction={() => setForm({ name: '', category: 'General', unit: 'Piece', openingStock: 0, currentStock: 0, minimumStock: 0, costPrice: 0, supplier: '', active: true })}><div className="module-toolbar"><div className="search-field">⌕<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search raw materials" /></div></div>{form && <SimpleForm title="Raw material" fields={['name', 'category', 'unit', 'openingStock', 'currentStock', 'minimumStock', 'costPrice', 'supplier']} form={form} setForm={setForm} onSave={save} onCancel={() => setForm(null)} />}{inventory.filter((item) => `${item.name} ${item.category}`.toLowerCase().includes(search.toLowerCase())).map((item) => <div className="module-row" key={item.id}><div><strong>{item.name}</strong><small>{item.category} · {item.unit} · {item.supplier || 'No supplier'}</small></div><span className={item.currentStock <= item.minimumStock ? 'low-stock' : ''}>{item.currentStock} {item.unit}{item.currentStock <= item.minimumStock && ' · LOW STOCK'}</span><b>{money(item.costPrice)}</b><button className="text-button" onClick={() => setForm(item)}>Edit</button><button className="text-button" onClick={() => setInventory((current) => current.filter((entry) => entry.id !== item.id))}>Delete</button></div>)}</ModuleFrame> }
function PurchasePage({ inventory, suppliers, purchases, savePurchase, notify }) { const [form, setForm] = useState(null); const open = () => setForm({ id: `PUR-${Date.now()}`, date: new Date().toISOString().slice(0, 10), supplier: suppliers[0]?.name || '', inventoryId: inventory[0]?.id || '', quantity: 1, rate: 0, gstRate: 0, discount: 0, paymentStatus: 'Unpaid' }); return <ModuleFrame title="Purchases" action="Add purchase" onAction={open}>{form && <div className="panel module-editor"><h2>New purchase</h2><SimpleForm title="Purchase details" fields={['id', 'date', 'supplier', 'inventoryId', 'quantity', 'rate', 'gstRate', 'discount', 'paymentStatus']} form={form} setForm={setForm} onSave={() => { savePurchase({ ...form, total: Number(form.quantity) * Number(form.rate) - Number(form.discount) }); setForm(null) }} onCancel={() => setForm(null)} /></div>}{purchases.map((purchase) => <div className="module-row" key={purchase.id}><strong>{purchase.id}</strong><span>{purchase.date}</span><span>{purchase.supplier}</span><span>{money(purchase.total || 0)}</span><span>{purchase.paymentStatus}</span></div>)}{!inventory.length && <p className="module-empty">Add raw materials before recording purchases.</p>}</ModuleFrame> }
...
function RecipePage({ menu, inventory, recipes, setRecipes, notify }) { const [menuId, setMenuId] = useState(menu[0]?.id); const [ingredient, setIngredient] = useState({ inventoryId: inventory[0]?.id || '', quantity: 1 }); const list = recipes[menuId] || []; const save = () => { setRecipes((current) => ({ ...current, [menuId]: [...(current[menuId] || []), { ...ingredient, quantity: Number(ingredient.quantity) }] })); notify('Ingredient added to recipe') }; return <ModuleFrame title="Recipes" action="Add ingredient" onAction={save}><label className="recipe-select">Menu item<Picker value={menuId} onChange={(value) => setMenuId(Number(value))} options={menu.map((item) => ({ value: item.id, label: item.name }))} /></label><div className="recipe-add"><Picker value={ingredient.inventoryId} onChange={(value) => setIngredient({ ...ingredient, inventoryId: value })} options={inventory.map((item) => ({ value: item.id, label: `${item.name} (${item.unit})` }))} /><input type="number" value={ingredient.quantity} onChange={(event) => setIngredient({ ...ingredient, quantity: event.target.value })} /><button className="button primary" onClick={save}>Add ingredient</button></div>{list.map((item, index) => <div className="module-row" key={`${item.inventoryId}-${index}`}><strong>{inventory.find((entry) => entry.id === item.inventoryId)?.name || 'Unknown item'}</strong><span>{item.quantity}</span><button className="text-button" onClick={() => setRecipes((current) => ({ ...current, [menuId]: current[menuId].filter((_, position) => position !== index) }))}>Remove</button></div>)}</ModuleFrame> }
