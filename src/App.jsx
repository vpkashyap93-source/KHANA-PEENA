...
function FestivalBanner({ festival, restaurantName }) { if (!festival) return null; return <div className={`festival-banner festival-${festival.theme}`}><span className="festival-emoji">{festival.emoji}</span><div><strong>{festival.message}</strong><small>From all of us at {restaurantName || 'Bojjan'}</small></div></div> }
...
const exportBackup = () => {
  ...
  link.href = url
  link.download = `bojjan-backup-${new Date().toISOString().slice(0, 10)}.json`
  link.click()
    ...
    <div className="bill-modal bill-size-80">
      ...
      <div className="print-bill">
        <header className="bill-header">
          <h1>{profile?.restaurantName || 'BOJJAN'}</h1>
...
function App() {
  ...
  return (
    <>
      ...
      <aside className={`sidebar ${mobileNav ? 'open' : ''}`}><div className="brand"><span className="brand-mark">{profile.restaurantName?.trim()?.[0]?.toUpperCase() || 'B'}</span><span><strong>{profile.restaurantName || 'BOJJAN'}</strong><small>RESTAURANT OS</small></span><button className="icon-button sidebar-close" onClick={() => setMobileNav(false)}>×</button></div><div className="workspace-label">WORKSPACE</div><nav>
  {navItems
    .filter(([label]) => {
...
function LoginSetup({ onSave }) {
  ...
  return <div className="registration-shell"><div className="registration-card">
    <div className="brand registration-brand"><span className="brand-mark">B</span><span><strong>BOJJAN</strong><small>RESTAURANT OS</small></span></div>
