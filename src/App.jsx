...
</nav> <div className="sidebar-bottom"><a className="help-card" href={SUPPORT_HELP_WHATSAPP_LINK} target="_blank" rel="noreferrer"><span>?</span><div><strong>Need a hand?</strong><small>Chat on WhatsApp · {SUPPORT_MOBILE_DISPLAY}</small></div></a><a className="help-card feedback-card" href={SUPPORT_FEEDBACK_WHATSAPP_LINK} target="_blank" rel="noreferrer"><span>💬</span><div><strong>Feedback &amp; suggestions</strong><small>Tell us what to improve</small></div></a><div className="profile"><div className="avatar">{ownerInitials}</div><div><strong>{ownerName}</strong><small>Administrator</small></div>{isAdmin && <button className="icon-button" onClick={() => setAdminViewMode('admin')} title="Back to Admin Dashboard">⌂</button>}<button className="icon-button logout-button" onClick={logOut} title="Log out">⎋</button></div></div></aside>
