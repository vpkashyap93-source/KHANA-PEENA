...
function BillPreview({ order, profile, onClose }) {
  ...
  return (
    <div className={`bill-modal bill-size-${paperSize.replace('mm', '')}`}>
      ...
      <div className="print-bill">
        <header className="bill-header">
          {profile?.logo && <img src={profile.logo} className="bill-logo" alt="" />}
          <h1>{profile?.restaurantName || 'YOUR RESTAURANT'}</h1>
          <p>{profile?.address || ''}</p>
          <p>
        ...
        <div className="bill-total">
          ...
          {order.gst > 0 && (
            <div><span>GST</span><strong>{money(order.gst)}</strong></div>
          )}
...
function App() {
  ...
  // Keeps different accounts logged into the same browser from seeing each other's local data.
  const [accountSyncing, setAccountSyncing] = useState(true)
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect
    if (!authUser) { setAccountSyncing(false); return }
    let cancelled = false
    setAccountSyncing(true)
    syncAccountData(authUser.uid).then((result) => {
      if (cancelled) return
      if (result.switched) { window.location.reload(); return }
  ...
  const [menuCategories, setMenuCategories] = useState(() => JSON.parse(localStorage.getItem('basil-menu-categories')) || [...new Set(menuCatalog.map((item) => item.category))])
  ...
  const [expenseCategories, setExpenseCategories] = useState(() => JSON.parse(localStorage.getItem('basil-expense-categories')) || ['Rent', 'Salaries', 'Utilities', 'Maintenance', 'Marketing', 'Supplies', 'Other'])
  const [capitalTransactions, setCapitalTransactions] = useState(() => JSON.parse(localStorage.getItem('basil-capital-transactions')) || [])
  ...
  useEffect(() => { localStorage.setItem('basil-menu-categories', JSON.stringify(menuCategories)) }, [menuCategories])
  ...
  useEffect(() => { localStorage.setItem('basil-expense-categories', JSON.stringify(expenseCategories)) }, [expenseCategories])
  useEffect(() => { localStorage.setItem('basil-capital-transactions', JSON.stringify(capitalTransactions)) }, [capitalTransactions])
  ...
  // Composition and Unregistered dealers cannot legally charge GST separately on the
  // bill (it's absorbed into the price), so GST is only ever charged when Regular.
  const gstChargeable = profile?.gstApplicable && (!profile?.registrationType || profile.registrationType === 'Regular')
  const gst = gstChargeable ? Math.round(cart.reduce((sum, item) => sum + item.price * item.quantity * (1 - discountPercent / 100) * (Number(profile?.standardGstRate) > 0 ? Number(profile.standardGstRate) : item.gstRate) / 100, 0)) : 0
  const gstRateLabel = Number(profile?.standardGstRate) > 0 ? `${profile.standardGstRate}%` : ''
  const total = subtotal - discount + gst
  useEffect(() => {
  document.body.dataset.gst = gstChargeable ? 'on' : 'off';
...
}, [profile, subtotal, discount, gst, gstChargeable]);
  ...
  // All billing works entirely from localStorage - this only tracks connectivity so staff
  // know their sales are still being saved, and to sync right away once back online instead
  // of waiting for the next 60s tick.
  useEffect(() => {
    const goOnline = () => { setIsOnline(true); notify("Back online - syncing…"); syncToCloud() }
    const goOffline = () => { setIsOnline(false); notify('Offline - your data is safe and saving locally') }
  ...
  // Corrects a wrongly-picked payment method (e.g. billed as Cash by mistake, was
  // actually UPI) after the sale is already done - updates the order and every
  // payment transaction recorded against it, so Cash/Bank totals in Reports stay correct.
  const changeOrderPaymentMethod = (orderId, method) => {
    setOrders((current) => current.map((order) => order.id === orderId && order.paidAmount > 0 ? { ...order, paymentMethod: method } : order))
    setPaymentTransactions((current) => current.map((transaction) => transaction.orderNumber === orderId ? { ...transaction, method } : transaction))
    notify('Payment method updated to ' + method)
  }
  useEffect(() => { window.basilChangePaymentMethod = changeOrderPaymentMethod; return () => { delete window.basilChangePaymentMethod } }, [])
  ...
  if (accountSyncing) return <AuthLoading />
...
</nav> <div className="sidebar-bottom"><a className="help-card" href={SUPPORT_HELP_WHATSAPP_LINK} target="_blank" rel="noreferrer"><span>?</span><div><strong>Help chahiye?</strong><small>Chat on WhatsApp · {SUPPORT_MOBILE_DISPLAY}</small></div></a><a className="help-card feedback-card" href={SUPPORT_FEEDBACK_WHATSAPP_LINK} target="_blank" rel="noreferrer"><span>💬</span><div><strong>Feedback &amp; suggestions</strong><small>Tell us what to improve</small></div></a><div className="profile"><div className="avatar">{ownerInitials}</div><div><strong>{ownerName}</strong><small>Administrator</small></div>{isAdmin && <button className="icon-button" onClick={() => setAdminViewMode('admin')} title="Back to Admin Dashboard">⌂</button>}<button className="icon-button logout-button" onClick={logOut} title="Log out">⎋</button></div></div></aside>
...
const SUPPORT_HELP_WHATSAPP_LINK = SUPPORT_WHATSAPP_LINK + '?text=' + encodeURIComponent('Hi, I need some help with my Shahi Bhoj software.')
const SUPPORT_FEEDBACK_WHATSAPP_LINK = SUPPORT_WHATSAPP_LINK + '?text=' + encodeURIComponent('Hi, I have some feedback / a suggestion for Shahi Bhoj:\n')
...
const blankProfile = { restaurantName: '', ownerName: '', mobile: '', email: '', address: '', city: '', state: '', pincode: '', logo: '', gstApplicable: true, gstin: '', registrationType: 'Regular', standardGstRate: '', compositionGstRate: '5', discountEnabled: true, discountPercent: 5, kitchenWorkflow: false, tableManagement: false, kotSystem: false, customerManagement: false, deliveryOrders: false, inventoryManagement: false }
...
function BusinessFields({ form, update }) { return <div className="settings-section"><div className="settings-section-title"><h2>Business details</h2><p>These details appear on your restaurant bills. Fields marked * are required.</p></div><div className="settings-form-grid"><label>Restaurant name *<input value={form.restaurantName} onChange={(event) => update('restaurantName', event.target.value)} placeholder="Shahi Bhoj" /></label><label>Owner name *<input value={form.ownerName} onChange={(event) => update('ownerName', event.target.value)} placeholder="Owner name" /></label><label>Mobile number *<input value={form.mobile} onChange={(event) => update('mobile', event.target.value)} placeholder="+91 98765 43210" /></label><label>Email<input type="email" value={form.email} onChange={(event) => update('email', event.target.value)} placeholder="hello@restaurant.com" /></label><label className="full-field">Address *<input value={form.address} onChange={(event) => update('address', event.target.value)} placeholder="Street and building" /></label><label>City *<input value={form.city} onChange={(event) => update('city', event.target.value)} placeholder="Mumbai" /></label><label>State *<input value={form.state} onChange={(event) => update('state', event.target.value)} placeholder="Maharashtra" /></label><label>Pincode *<input value={form.pincode} onChange={(event) => update('pincode', event.target.value)} placeholder="400001" /></label><label className="full-field logo-upload-field">Restaurant logo (optional - printed on your bills)<div className="logo-upload-row">{form.logo && <img src={form.logo} className="logo-preview" alt="Logo preview" />}<input type="file" accept="image/*" onChange={(event) => { const file = event.target.files[0]; if (!file) return; resizeImage(file, 240, (dataUrl) => update('logo', dataUrl)) }} />{form.logo && <button type="button" className="button quiet" onClick={() => update('logo', '')}>Remove</button>}</div></label></div></div> }
function GstFields({ form, update }) { return <div className="settings-section gst-settings"><div className="settings-section-title"><div><h2>GST configuration</h2><p>GST is controlled at restaurant level for every bill.</p></div><button className={`gst-toggle ${form.gstApplicable ? 'on' : ''}`} onClick={() => update('gstApplicable', !form.gstApplicable)}><span />GST {form.gstApplicable ? 'ON' : 'OFF'}</button></div>{form.gstApplicable && <div className="settings-form-grid"><label>GSTIN<input value={form.gstin} onChange={(event) => update('gstin', event.target.value.toUpperCase())} placeholder="27ABCDE1234F1Z5" /></label><label>GST registration type<Picker value={form.registrationType} onChange={(value) => update('registrationType', value)} options={['Regular', 'Composition', 'Unregistered']} /></label>{form.registrationType === 'Regular' && <label>Standard GST rate (%) - leave blank to use each item's own rate<input type="number" min="0" max="28" value={form.standardGstRate} onChange={(event) => update('standardGstRate', event.target.value)} placeholder="e.g. 5" /></label>}{form.registrationType === 'Composition' && <label>Composition GST rate (%) - fixed rate you pay on total sales<input type="number" min="0" max="28" value={form.compositionGstRate} onChange={(event) => update('compositionGstRate', event.target.value)} placeholder="e.g. 5" /></label>}</div>}{form.registrationType === 'Composition' && <p className="gst-note">Composition scheme: GST will not be shown or charged separately on bills - it stays included in your prices, as required by law. You still owe the government {form.compositionGstRate || 0}% of your total sales - Reports will show this as your GST payable.</p>}{form.registrationType === 'Unregistered' && <p className="gst-note">Unregistered: GST will not be charged on bills.</p>}</div> }
...
function Dashboard({ navigate, orders, tables, customers, inventory = [], profile, now, festival, ownerName, restaurantName }) {
  const hour = now.getHours()
  const greeting = hour < 12 ? 'Good morning' : hour < 17 ? 'Good afternoon' : 'Good evening'
  const todayOrders = orders.filter((order) => order.createdAt && new Date(order.createdAt).toDateString() === now.toDateString())
  const emailTodaySummary = () => {
...
function PartyLedgerPage({ parties, setParties, transactions, setTransactions, suppliers = [], purchases = [], paySupplier, inventory = [], profile, notify }) {
  ...
  const saveTxn = () => {
    const txn = { id: `txn-${Date.now()}`, partyId: selectedEntity.id, type: txnForm.type, amount: Number(txnForm.amount) || 0, note: txnForm.note || '', date: new Date().toISOString().slice(0, 10) };
    setTransactions((current) => [txn, ...current]);
    setTxnForm(null);
    notify('Entry added');
  };

  const supplierRows = computeSupplierRows(suppliers, purchases);
  const totalReceivable = parties.reduce((sum, item) => sum + Math.max(0, balanceOf(item.id)), 0);
  const totalPayable = parties.reduce((sum, item) => sum + Math.max(0, -balanceOf(item.id)), 0) + supplierRows.reduce((sum, item) => sum + item.outstanding, 0);

  const combinedRows = [
    ...parties.map((item) => ({ type: 'party', id: item.id, name: item.name, sub: item.mobile || '—', balance: balanceOf(item.id) })),
    ...supplierRows.map((item) => ({ type: 'supplier', id: item.name, name: item.name, sub: 'Supplier', balance: -item.outstanding }))
  ...
  if (selectedEntity?.type === 'party') {
    const party = parties.find((item) => item.id === selectedEntity.id);
    const balance = balanceOf(selectedEntity.id);
    const rows = transactions.filter((item) => item.partyId === selectedEntity.id).sort((a, b) => (a.id < b.id ? 1 : -1));

    return (
      <>
        <PageTitle eyebrow="PARTY LEDGER" title={party.name} action="Back" onAction={() => setSelectedEntity(null)} />
  ...
  return (
    <>
      <PageTitle eyebrow="LEDGER" title="Party &amp; Supplier Ledger" action="Add party" onAction={() => setForm({ name: '', mobile: '', openingBalance: 0 })} />

      <div className="summary-grid">
        <article className="summary-card"><span>Total Parties &amp; Suppliers</span><strong>{parties.length + suppliers.length}</strong></article>
      ...
      <section className="panel">
        <div className="module-toolbar"><div className="search-field">⌕<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search anyone - party or supplier" /></div></div>
        <div className="table-wrap">
          <table className="data-table">
            <thead><tr><th>Name</th><th>Type</th><th>Balance</th><th>View</th></tr></thead>
            <tbody>
              {combinedRows.length ? combinedRows.map((item) => (
                <tr key={`${item.type}-${item.id}`}>
                  <td><strong>{item.name}</strong></td>
                  <td>{item.sub}</td>
                  <td><strong className={item.balance >= 0 ? 'accounting-paid' : 'accounting-due'}>{money(Math.abs(item.balance))} {item.balance >= 0 ? '(Get)' : '(Give)'}</strong></td>
                  <td><button className="button secondary" onClick={() => setSelectedEntity({ type: item.type, id: item.id })}>View Ledger</button></td>
                </tr>
              )) : <tr><td colSpan="4">No parties or suppliers added yet.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
...
function MenuPage({ menu, setMenu, categories, setCategories, profile, notify }) { const [search, setSearch] = useState(''); const [form, setForm] = useState(null); const [quickAddCategory, setQuickAddCategory] = useState(false); const colors = ['coral', 'gold', 'green', 'cream', 'orange', 'yellow', 'brown', 'pink']; const itemGstRateNeeded = profile?.gstApplicable && (!profile?.registrationType || profile.registrationType === 'Regular') && !(Number(profile?.standardGstRate) > 0); const save = () => { const item = { ...form, id: form.id || Date.now(), price: Number(form.price) || 0, gstRate: Number(form.gstRate) || 0, available: form.available !== false }; setMenu((current) => current.some((entry) => entry.id === item.id) ? current.map((entry) => entry.id === item.id ? item : entry) : [...current, item]); setForm(null); notify('Menu item saved') }; const addCategoryInline = (values) => { if (!values.name?.trim()) return; const name = values.name.trim(); if (!categories.includes(name)) setCategories((current) => [...current, name]); setForm((current) => current && { ...current, category: name }); setQuickAddCategory(false); notify('Category added') }; const pickImage = (event) => { const file = event.target.files[0]; if (!file) return; resizeImage(file, 480, (dataUrl) => setForm((current) => ({ ...current, image: dataUrl }))) }; return <ModuleFrame title="Menu / Items" action="Add menu item" onAction={() => setForm({ name: '', category: categories[0] || 'Starters', price: 0, type: 'Veg', gstRate: 5, color: 'coral', available: true, image: '' })}><div className="module-toolbar"><div className="search-field">⌕<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search menu items" /></div></div>{form && <><div className="module-quick-add"><button type="button" className="text-button" onClick={() => setQuickAddCategory(true)}>+ New category</button></div><div className="panel module-editor"><h2>{form.id ? 'Edit menu item' : 'New menu item'}</h2><div className="settings-form-grid"><label>Name<input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label>Category<Picker value={form.category} onChange={(value) => setForm({ ...form, category: value })} options={categories} /></label><label>Price<input type="number" value={form.price} onChange={(event) => setForm({ ...form, price: event.target.value })} /></label>{itemGstRateNeeded && <label>GST rate (%)<input type="number" value={form.gstRate} onChange={(event) => setForm({ ...form, gstRate: event.target.value })} /></label>}<label>Type<Picker value={form.type} onChange={(value) => setForm({ ...form, type: value })} options={['Veg', 'Non-veg']} /></label><label>Card color<Picker value={form.color} onChange={(value) => setForm({ ...form, color: value })} options={colors} /></label><label className="full-field">Item photo<input type="file" accept="image/*" onChange={pickImage} />{form.image && <div className="photo-preview"><img src={form.image} alt="Preview" /><button type="button" className="text-button" onClick={() => setForm({ ...form, image: '' })}>Remove photo</button></div>}</label></div><div className="cart-actions"><button className="button secondary" onClick={() => setForm(null)}>Cancel</button><button className="button primary" onClick={save}>Save item</button></div></div></>}{quickAddCategory && <PromptDialog title="New menu category" fields={[{ key: 'name', label: 'Category name', placeholder: 'e.g. Starters' }]} onConfirm={addCategoryInline} onCancel={() => setQuickAddCategory(false)} />}{menu.filter((item) => `${item.name} ${item.category}`.toLowerCase().includes(search.toLowerCase())).map((item) => <div className="module-row" key={item.id}><div className="module-row-title">{item.image ? <img src={item.image} alt={item.name} className="module-thumb" /> : <span className={`item-thumb ${item.color}`}>{item.name.slice(0, 1)}</span>}<div><strong>{item.name}</strong><small>{item.category} · {item.type}{itemGstRateNeeded && ` · GST ${item.gstRate}%`}</small></div></div><b>{money(item.price)}</b><span className={item.available === false ? 'low-stock' : ''}>{item.available === false ? 'Unavailable' : 'Available'}</span><button className="text-button" onClick={() => setForm(item)}>Edit</button><button className="text-button" onClick={() => setMenu((current) => current.map((entry) => entry.id === item.id ? { ...entry, available: entry.available === false } : entry))}>{item.available === false ? 'Enable' : 'Disable'}</button><button className="text-button" onClick={() => setMenu((current) => current.filter((entry) => entry.id !== item.id))}>Delete</button></div>)}</ModuleFrame> }
...
function ExpensePage({ expenses, setExpenses, categories, setCategories, profile, notify }) {
  ...
  const save = () => {
    const item = { ...form, id: form.id || `exp-${Date.now()}`, amount: Number(form.amount) || 0 }
    setExpenses((current) => current.some((entry) => entry.id === item.id) ? current.map((entry) => entry.id === item.id ? item : entry) : [item, ...current])
    setForm(null)
    notify('Expense saved')
  }

  const addCategoryInline = (values) => {
    if (!values.name?.trim()) return
    const name = values.name.trim()
    if (!categories.includes(name)) setCategories((current) => [...current, name])
    setForm((current) => current && { ...current, category: name })
  ...
  const categoryRows = categories.map((cat) => {
    ...
    return { name: cat, total: catExpenses.reduce((sum, item) => sum + Number(item.amount || 0), 0), count: catExpenses.length }
  })

  if (selectedCategory) {
    const catExpenses = expenses.filter((item) => item.category === selectedCategory)
    const catTotal = catExpenses.reduce((sum, item) => sum + Number(item.amount || 0), 0)
    return (
      <>
        <PageTitle eyebrow="EXPENSES" title={selectedCategory} action="Back" onAction={() => setSelectedCategory(null)} />
        <div className="summary-grid">
        ...
        <section className="panel">
          ...
          <div className="table-wrap">
            <table className="data-table">
              <thead><tr><th>Date</th><th>Description</th><th>Payment</th><th>Amount</th></tr></thead>
              <tbody>
                {catExpenses.length ? catExpenses.map((item) => (
                  <tr key={item.id}><td>{item.date}</td><td>{item.description || '—'}</td><td>{item.paymentMethod}</td><td>{money(item.amount)}</td></tr>
                )) : <tr><td colSpan="4">No entries yet.</td></tr>}
              </tbody>
            </table>
          </div>
  ...
  return <ModuleFrame title="Expenses" action="Add expense" onAction={() => setForm({ date: new Date().toISOString().slice(0, 10), category: categories[0] || 'Other', description: '', amount: 0, paymentMethod: 'Cash' })}>
    <div className="accounting-summary"><div><span>Total expenses</span><strong>{money(total)}</strong></div></div>
    {form && <><div className="module-quick-add"><button type="button" className="text-button" onClick={() => setQuickAddCategory(true)}>+ New category</button></div><div className="panel module-editor"><h2>{form.id ? 'Edit expense' : 'New expense'}</h2><div className="settings-form-grid"><label>Date<input type="date" value={form.date} onChange={(event) => setForm({ ...form, date: event.target.value })} /></label><label>Category<Picker value={form.category} onChange={(value) => setForm({ ...form, category: value })} options={categories} /></label><label>Description<input value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label><label>Amount<input type="number" value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} /></label><label>Payment method<Picker value={form.paymentMethod} onChange={(value) => setForm({ ...form, paymentMethod: value })} options={['Cash', 'UPI', 'Card', 'Bank Transfer']} /></label></div><div className="cart-actions"><button className="button secondary" onClick={() => setForm(null)}>Cancel</button><button className="button primary" onClick={save}>Save expense</button></div></div></>}
    {quickAddCategory && <PromptDialog title="New expense category" fields={[{ key: 'name', label: 'Category name', placeholder: 'e.g. Insurance' }]} onConfirm={addCategoryInline} onCancel={() => setQuickAddCategory(false)} />}

    <section className="panel">
      <div className="panel-heading"><div><span className="eyebrow">BY CATEGORY</span><h2>Expense categories</h2></div></div>
      {categoryRows.map((cat) => <div className="module-row" key={cat.name}><strong>{cat.name}</strong><span>{cat.count} entr{cat.count === 1 ? 'y' : 'ies'}</span><b>{money(cat.total)}</b><button className="text-button" onClick={() => setSelectedCategory(cat.name)}>View</button></div>)}
    </section>

    {expenses.length ? expenses.map((item) => <div className="module-row" key={item.id}><div><strong>{item.description || item.category}</strong><small>{item.category} · {item.date}</small></div><b>{money(item.amount)}</b><span>{item.paymentMethod}</span><button className="text-button" onClick={() => setForm(item)}>Edit</button><button className="text-button" onClick={() => setExpenses((current) => current.filter((entry) => entry.id !== item.id))}>Delete</button></div>) : <p className="module-empty">No expenses recorded yet.</p>}
  </ModuleFrame>
}
      ...
      <div className="print-bill print-report">
        ...
        <table className="report-print-table">
          ...
          <tbody>
            ...
            <tr><td>Fixed assets</td><td>{money(balanceSheet.fixedAssetsValue)}</td></tr>
...
function ReportsPage({ orders, inventory, expenses = [], parties = [], partyTransactions = [], paymentTransactions = [], purchases = [], suppliers = [], profile, capitalTransactions = [], addCapitalTransaction }) {
  const totalSales = orders.reduce((sum, order) => sum + Number(order.amount || 0), 0)
  ...
  // Input Tax Credit: only claimable when this restaurant is itself Regular-registered, and
  // only on purchases from Regular-registered suppliers (Composition/unregistered suppliers
  // don't charge GST separately, so there's nothing to claim back).
  const suppliersByName = Object.fromEntries(suppliers.map((supplier) => [supplier.name, supplier]))
  const ownRegular = !profile?.registrationType || profile.registrationType === 'Regular'
  const totalInputGst = ownRegular ? purchases.reduce((sum, purchase) => suppliersByName[purchase.supplier]?.registrationType === 'Regular' ? sum + (Number(purchase.total) || 0) * (Number(purchase.gstRate) || 0) / 100 : sum, 0) : 0
  // Composition dealers can't claim ITC - they owe a flat % of total turnover instead,
  // regardless of how much GST (if any) shows up on individual bills.
  const isComposition = profile?.registrationType === 'Composition'
  const compositionLiability = isComposition ? Math.round(totalSales * (Number(profile?.compositionGstRate) || 0) / 100) : 0
  const netGstPayable = isComposition ? compositionLiability : (totalGst - totalInputGst)
  ...
  const openingStockValue = inventory.reduce((sum, item) => sum + Number(item.openingStock || 0) * Number(item.costPrice || 0), 0)
  const totalPurchaseValue = purchases.reduce((sum, purchase) => sum + Number(purchase.total || 0), 0)
  const cogs = openingStockValue + totalPurchaseValue - totalStockValue
  const grossProfit = totalSales - cogs
  const netProfitPL = grossProfit - totalExpenses

  const cashIn = paymentTransactions.filter((item) => item.method === 'Cash').reduce((sum, item) => sum + Number(item.amount || 0), 0)
  const bankIn = paymentTransactions.filter((item) => item.method === 'UPI' || item.method === 'Card').reduce((sum, item) => sum + Number(item.amount || 0), 0)
  const cashOutExpenses = expenses.filter((item) => (item.paymentMethod || 'Cash') === 'Cash').reduce((sum, item) => sum + Number(item.amount || 0), 0)
  const bankOutExpenses = expenses.filter((item) => (item.paymentMethod || 'Cash') !== 'Cash').reduce((sum, item) => sum + Number(item.amount || 0), 0)
  const cashOutPurchases = purchases.filter((item) => item.paymentMethod === 'Cash').reduce((sum, item) => sum + Number(item.paidAmount || 0), 0)
  const bankOutPurchases = purchases.filter((item) => item.paymentMethod === 'UPI' || item.paymentMethod === 'Card').reduce((sum, item) => sum + Number(item.paidAmount || 0), 0)

  const capitalIntroduced = capitalTransactions.filter((item) => item.type === 'Capital Introduced').reduce((sum, item) => sum + Number(item.amount || 0), 0)
  const drawings = capitalTransactions.filter((item) => item.type === 'Drawings').reduce((sum, item) => sum + Number(item.amount || 0), 0)
  const fixedAssetsValue = capitalTransactions.filter((item) => item.type === 'Fixed Asset Purchase').reduce((sum, item) => sum + Number(item.amount || 0), 0)
  const cashInCapital = capitalTransactions.filter((item) => item.type === 'Capital Introduced' && item.method === 'Cash').reduce((sum, item) => sum + Number(item.amount || 0), 0)
  const bankInCapital = capitalTransactions.filter((item) => item.type === 'Capital Introduced' && item.method !== 'Cash').reduce((sum, item) => sum + Number(item.amount || 0), 0)
  const cashOutCapital = capitalTransactions.filter((item) => item.type !== 'Capital Introduced' && item.method === 'Cash').reduce((sum, item) => sum + Number(item.amount || 0), 0)
  const bankOutCapital = capitalTransactions.filter((item) => item.type !== 'Capital Introduced' && item.method !== 'Cash').reduce((sum, item) => sum + Number(item.amount || 0), 0)

  const cashBalance = cashIn + cashInCapital - cashOutExpenses - cashOutPurchases - cashOutCapital
  const bankBalance = bankIn + bankInCapital - bankOutExpenses - bankOutPurchases - bankOutCapital

  const supplierOutstanding = purchases.reduce((sum, purchase) => sum + Number(purchase.outstandingAmount ?? (purchase.total - (purchase.paidAmount || 0))), 0)
  const totalAssets = cashBalance + bankBalance + totalStockValue + totalOutstanding + fixedAssetsValue
  ...
  const exportToExcel = async () => {
    ...
    addSheet('Summary', [
      ...
      ['Total expenses', totalExpenses], ['Net profit', netProfit], ['GST collected (Output)', totalGst], ['Input GST (ITC)', totalInputGst], [isComposition ? 'Composition GST payable' : 'Net GST payable', netGstPayable], ['Discount given', totalDiscount],
    ...
    addSheet('Balance Sheet', [
      ...
      ['', '', '', 'Fixed assets', fixedAssetsValue],
      ['', '', '', 'Sundry debtors (customers)', totalOutstanding],
      ['Total', totalLiabilities + capital, '', 'Total', totalAssets],
    ])
    addSheet('Capital Account', [
      ['Capital Account', reportDate], [],
      ['Capital introduced', capitalIntroduced], ['Drawings', drawings], ['Fixed assets bought', fixedAssetsValue], [],
      ['Date', 'Type', 'Note', 'Method', 'Amount'],
      ...capitalTransactions.map((item) => [item.date, item.type, item.note, item.method, item.amount]),
    ])
  ...
  if (cashBankView) {
    ...
    const salesRows = paymentTransactions.filter((item) => isCash ? item.method === 'Cash' : (item.method === 'UPI' || item.method === 'Card'))
    const capitalInRows = capitalTransactions.filter((item) => item.type === 'Capital Introduced' && (isCash ? item.method === 'Cash' : item.method !== 'Cash'))
    const expenseOutRows = expenses.filter((item) => isCash ? (item.paymentMethod || 'Cash') === 'Cash' : (item.paymentMethod || 'Cash') !== 'Cash')
    const supplierOutRows = purchases.filter((item) => Number(item.paidAmount || 0) > 0 && (isCash ? item.paymentMethod === 'Cash' : (item.paymentMethod === 'UPI' || item.paymentMethod === 'Card')))
    const capitalOutRows = capitalTransactions.filter((item) => item.type !== 'Capital Introduced' && (isCash ? item.method === 'Cash' : item.method !== 'Cash'))
    ...
    return (
      <>
        ...
        {capitalInRows.length > 0 && <section className="panel"><div className="panel-heading"><div><span className="eyebrow">MONEY IN</span><h2>Capital introduced</h2></div></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Date</th><th>Note</th><th>Amount</th></tr></thead><tbody>{capitalInRows.map((item) => <tr key={item.id}><td>{item.date}</td><td>{item.note || '—'}</td><td>{money(item.amount)}</td></tr>)}</tbody></table></div></section>}

        <section className="panel"><div className="panel-heading"><div><span className="eyebrow">MONEY OUT</span><h2>Expenses paid</h2></div></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Date</th><th>Category</th><th>Amount</th></tr></thead><tbody>{expenseOutRows.length ? expenseOutRows.map((item) => <tr key={item.id}><td>{item.date}</td><td>{item.description || item.category}</td><td>{money(item.amount)}</td></tr>) : <tr><td colSpan="3">None yet.</td></tr>}</tbody></table></div></section>
  ...
  return <>
    ...
    {printPreview && (
      <ReportPrintPreview
        ...
        balanceSheet={{ totalLiabilities, capital, cashBalance, bankBalance, totalStockValue, fixedAssetsValue, totalOutstanding, totalAssets }}
    ...
    <div className="summary-grid">
      ...
      <article className="summary-card"><span>GST collected (Output)</span><strong>{money(totalGst)}</strong></article>
      {ownRegular && !isComposition && <article className="summary-card"><span>Input GST (ITC)</span><strong>{money(totalInputGst)}</strong></article>}
      {profile?.registrationType !== 'Unregistered' && <article className="summary-card"><span>{isComposition ? `Composition GST payable (${Number(profile?.compositionGstRate) || 0}% of sales)` : 'Net GST payable'}</span><strong className={netGstPayable > 0 ? 'accounting-due' : 'accounting-paid'}>{money(netGstPayable)}</strong></article>}
    ...
    <section className="panel"><div className="panel-heading"><div><span className="eyebrow">FINANCIAL STATEMENTS</span><h2>Trading account</h2></div></div><div className="table-wrap"><table className="data-table"><tbody>
      <tr><td>Opening stock</td><td>{money(openingStockValue)}</td></tr>
      <tr><td>Add: Purchases</td><td>{money(totalPurchaseValue)}</td></tr>
      <tr><td>Less: Closing stock</td><td>{money(totalStockValue)}</td></tr>
      <tr><td><strong>Cost of goods sold</strong></td><td><strong>{money(cogs)}</strong></td></tr>
      <tr><td>Sales</td><td>{money(totalSales)}</td></tr>
      <tr><td><strong>Gross profit</strong></td><td><strong className={grossProfit >= 0 ? 'accounting-paid' : 'accounting-due'}>{money(grossProfit)}</strong></td></tr>
    </tbody></table></div></section>

    <section className="panel"><div className="panel-heading"><div><span className="eyebrow">FINANCIAL STATEMENTS</span><h2>Profit &amp; loss account</h2></div></div><div className="table-wrap"><table className="data-table"><tbody>
      <tr><td>Gross profit</td><td>{money(grossProfit)}</td></tr>
      <tr><td>Less: Total expenses</td><td>{money(totalExpenses)}</td></tr>
      <tr><td><strong>Net profit</strong></td><td><strong className={netProfitPL >= 0 ? 'accounting-paid' : 'accounting-due'}>{money(netProfitPL)}</strong></td></tr>
    </tbody></table></div></section>

    <section className="panel"><div className="panel-heading"><div><span className="eyebrow">FINANCIAL STATEMENTS</span><h2>Balance sheet</h2></div><span>As of today</span></div>
      <div className="balance-sheet-grid">
        <div><h3>Liabilities</h3><table className="data-table"><tbody>
          <tr><td>Sundry creditors (suppliers)</td><td>{money(totalLiabilities)}</td></tr>
          <tr><td>Capital &amp; reserves</td><td>{money(capital)}</td></tr>
          <tr className="balance-sheet-total"><td><strong>Total</strong></td><td><strong>{money(totalLiabilities + capital)}</strong></td></tr>
        </tbody></table></div>
        <div><h3>Assets</h3><table className="data-table"><tbody>
          <tr><td>Cash in hand</td><td>{money(cashBalance)}</td></tr>
          <tr><td>Bank balance</td><td>{money(bankBalance)}</td></tr>
          <tr><td>Closing stock</td><td>{money(totalStockValue)}</td></tr>
          <tr><td>Fixed assets</td><td>{money(fixedAssetsValue)}</td></tr>
          <tr><td>Sundry debtors (customers)</td><td>{money(totalOutstanding)}</td></tr>
          <tr className="balance-sheet-total"><td><strong>Total</strong></td><td><strong>{money(totalAssets)}</strong></td></tr>
        </tbody></table></div>
      </div>
    </section>
    ...
    <section className="panel">
      ...
      <div className="cart-actions" style={{ justifyContent: 'flex-start', gap: 12, marginBottom: 18 }}>
        ...
        <button className="button secondary" onClick={() => setCapitalDialog('Drawings')}>+ Drawings</button>
        <button className="button secondary" onClick={() => setCapitalDialog('Fixed Asset Purchase')}>+ Fixed Asset Purchase</button>
      </div>
      {capitalDialog && (
        <PromptDialog
          title={capitalDialog}
          ...
          onConfirm={(values) => { addCapitalTransaction(capitalDialog, values.amount, values.method || 'Cash', values.note); setCapitalDialog(null) }}
        />
      )}
      <div className="table-wrap">
        <table className="data-table">
          <thead><tr><th>Date</th><th>Type</th><th>Note</th><th>Method</th><th>Amount</th></tr></thead>
          <tbody>
            {capitalTransactions.length ? capitalTransactions.map((item) => (
              <tr key={item.id}><td>{item.date}</td><td>{item.type}</td><td>{item.note || '—'}</td><td>{item.method}</td><td>{money(item.amount)}</td></tr>
            )) : <tr><td colSpan="5">No capital account entries yet.</td></tr>}
          </tbody>
        </table>
      </div>
    </section>
...
const SIMPLE_FORM_LABELS = { inventoryId: 'Raw material', gstRate: 'GST %', gstin: 'GSTIN', id: 'ID', registrationType: 'GST type (for input tax credit)' }
...
function SupplierPage({ suppliers, setSuppliers, purchases, paySupplier, inventory, profile, notify }) {
  ...
  return (
    <ModuleFrame title="Suppliers" action="Add supplier" onAction={() => setForm({ name: '', mobile: '', address: '', gstin: '', registrationType: 'Regular', openingBalance: 0 })}>
      {form && <SimpleForm title="Supplier" fields={['name', 'mobile', 'address', 'gstin', 'registrationType', 'openingBalance']} form={form} setForm={setForm} onSave={save} onCancel={() => setForm(null)} selects={{ registrationType: ['Regular', 'Composition', 'Unregistered'] }} />}
...
function OrderDetailViewActive({ orders, onViewBill }) { const [filter, setFilter] = useState('All'); const [selected, setSelected] = useState(null); const [amount, setAmount] = useState(''); const [method, setMethod] = useState('Cash'); const orderStatuses = ['New', 'Confirmed', 'Preparing', 'Ready', 'Served', 'Completed', 'Cancelled']; const changeStatus = (order, status) => { window.basilUpdateOrderStatus?.(order.id, status); setSelected({ ...order, status }) }; const collect = () => { if (selected) window.basilCollectPayment?.(selected, amount, method); setAmount(''); setSelected(null) }; const rows = orders.filter((order) => filter === 'All' || (order.status || 'New') === filter); return <><PageTitle eyebrow="ORDER MANAGEMENT" title="Orders" action="New order" /><div className="filter-tabs">{['All', ...orderStatuses].map((item) => <button className={filter === item ? 'active' : ''} onClick={() => setFilter(item)} key={item}>{item}</button>)}</div><section className="panel order-table"><div className="table-wrap"><table><thead><tr><th>ORDER</th><th>DATE / TIME</th><th>CUSTOMER</th><th>TABLE / TYPE</th><th>ORDER STATUS</th><th>PAYMENT</th><th>TOTAL</th><th>PAID</th><th>OUTSTANDING</th><th /><th /></tr></thead><tbody>{rows.map((order) => <tr key={order.id}><td><strong>{order.id}</strong><small>{order.kotNumber || 'KOT pending'}</small></td><td>{order.time}</td><td>{order.customer}</td><td>{order.table}<small>{order.orderType || 'Dine-in'}</small></td><td><span className={`badge ${(order.status || 'New').toLowerCase()}`}>{order.status || 'New'}</span></td><td><span className={`badge ${(order.paymentStatus || 'Unpaid').toLowerCase().replace(' ', '-')}`}>{order.paymentStatus || 'Unpaid'}</span></td><td><strong>{money(order.amount)}</strong></td><td>{money(order.paidAmount || 0)}</td><td>{money(order.outstandingAmount ?? order.amount)}</td><td><button className="text-button" onClick={() => { setSelected(order); setAmount(String(order.outstandingAmount ?? order.amount)) }}>Open / View</button></td><td><button className="text-button" onClick={() => onViewBill?.(order)}>View Bill</button></td></tr>)}</tbody></table></div></section>{selected && <div className="payment-dialog order-detail-overlay"><div className="panel order-detail"><div className="panel-head"><div><span className="eyebrow">ORDER DETAIL</span><h2>{selected.id}</h2></div><button className="icon-button" onClick={() => setSelected(null)}>×</button></div><div className="detail-grid"><span>Customer <b>{selected.customer}</b></span><span>Date/time <b>{selected.time}</b></span><span>Table <b>{selected.table}</b></span><span>Order type <b>{selected.orderType}</b></span><span>Order status <b>{selected.status || 'New'}</b></span><span>Kitchen/KOT <b>{selected.kotNumber || 'Pending'}</b></span></div><div className="detail-items">{selected.items?.map((item) => <div key={item.id}><span>{item.quantity} × {item.name}</span><span>{money(item.price)} · {money(item.price * item.quantity)}</span></div>)}</div><div className="detail-totals"><span>Subtotal <b>{money(selected.subtotal ?? selected.items?.reduce((sum, item) => sum + item.price * item.quantity, 0) ?? selected.amount)}</b></span><span>Discount <b>{money(selected.discount || 0)}</b></span><span>GST <b>{money(selected.gst || 0)}</b></span><strong>Grand total <b>{money(selected.amount)}</b></strong><span>Paid amount <b>{money(selected.paidAmount || 0)}</b></span><span>Outstanding <b>{money(selected.outstandingAmount ?? selected.amount)}</b></span></div><h3>Order actions</h3><div className="detail-actions"><button className="button secondary" onClick={() => onViewBill?.(selected)}>View / Print Bill</button>{orderStatuses.map((status) => <button className="button secondary" disabled={selected.status === status} onClick={() => changeStatus(selected, status)} key={status}>{status === 'Confirmed' ? 'Send to Kitchen' : status === 'Preparing' ? 'Start Preparing' : status === 'Ready' ? 'Mark Ready' : status === 'Served' ? 'Mark Served' : status === 'Completed' ? 'Complete Order' : status === 'Cancelled' ? 'Cancel Order' : 'Open Order'}</button>)}</div>{(selected.paidAmount || 0) > 0 && <div className="collect-box"><h3>Payment method</h3><small>Billed with the wrong method by mistake? Change it here.</small><Picker value={selected.paymentMethod || 'Cash'} onChange={(value) => { window.basilChangePaymentMethod?.(selected.id, value); setSelected((current) => current && { ...current, paymentMethod: value }) }} options={['Cash', 'UPI', 'Card']} /></div>}{(selected.outstandingAmount ?? selected.amount) > 0 && <div className="collect-box"><h3>Collect Payment</h3><small>Outstanding: {money(selected.outstandingAmount ?? selected.amount)}</small><input type="number" min="1" max={selected.outstandingAmount ?? selected.amount} value={amount} onChange={(event) => setAmount(event.target.value)} /><Picker value={method} onChange={setMethod} options={['Cash', 'UPI', 'Card']} /><button className="button primary" onClick={collect}>Record Payment</button></div>}</div></div>}</> }
